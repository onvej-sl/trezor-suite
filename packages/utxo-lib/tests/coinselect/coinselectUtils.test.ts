...
describe('coinselectUtils', () => {
    ...
    it('getDustAmount', () => {
        ...
        expect(getDustAmount({ txType: 'p2pkh' })).toEqual(148 * 3);
        expect(getDustAmount({ txType: 'p2sh' })).toEqual(91 * 3);
        expect(getDustAmount({ txType: 'p2tr' })).toEqual(58 * 3);
        expect(getDustAmount({ txType: 'p2wpkh' })).toEqual(68 * 3);
        ...
        expect(getDustAmount({ txType: 'p2pkh', longTermFeeRate: 5 })).toEqual(148 * 5);
        expect(getDustAmount({ txType: 'p2sh', longTermFeeRate: 5 })).toEqual(91 * 5);
        expect(getDustAmount({ txType: 'p2tr', longTermFeeRate: 5 })).toEqual(58 * 5);
        expect(getDustAmount({ txType: 'p2wpkh', longTermFeeRate: 5 })).toEqual(68 * 5);

        // with explicit dustThreshold and longTermFeeRate
        expect(getDustAmount({ txType: 'p2pkh', dustThreshold: 546, longTermFeeRate: 5 })).toEqual(
            740,
        ); // longTermFeeRate makes it higher than dustThreshold
        expect(getDustAmount({ txType: 'p2sh', dustThreshold: 546, longTermFeeRate: 5 })).toEqual(
            546,
        ); // dustThreshold is higher than 91 * 5 (455)
        expect(getDustAmount({ txType: 'p2tr', dustThreshold: 546, longTermFeeRate: 5 })).toEqual(
            546,
        ); // dustThreshold is higher than 58 * 5 (290)
        expect(getDustAmount({ txType: 'p2wpkh', dustThreshold: 546, longTermFeeRate: 5 })).toEqual(
            546,
        ); // dustThreshold is higher than 68 * 5 (340)

        // DOGE with high dustThreshold
        expect(
            getDustAmount({ txType: 'p2pkh', dustThreshold: 100000000, longTermFeeRate: 5 }),
